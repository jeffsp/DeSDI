...
/**
 * @brief Groups a vector of GeoPoints into spatial shards based on a Cubed-Sphere Hilbert index.
 * 
 ...
 */
inline std::map<uint64_t, Shard> shard_points(const std::vector<GeoPoint>& points, int level = 13) {
    ...
    for (const auto& pt : points) {
        ...
        uint64_t composite_key = (static_cast<uint64_t>(proj.face) << (2 * level)) | h;
