...
TEST_CASE ("Cubed-sphere projection at boundaries", "[cubed_sphere]")
{
    ...
    {
        REQUIRE_THAT (res.u,
                      WithinAbs (1.0, margin)); // y/x = 1 -> normalized to 1
        REQUIRE_THAT (res.v,
                      WithinAbs (0.5, margin)); // z/x = 0 -> normalized to 0.5
    }
    else
    {
        REQUIRE_THAT (res.u,
                      WithinAbs (0.0, margin)); // -x/y = -1 -> normalized to 0
        REQUIRE_THAT (res.v,
                      WithinAbs (0.5, margin)); // z/y = 0 -> normalized to 0.5
    }
}
